@echo off
REM Build script for fps_camera example
...
echo === EDEN ENGINE Build - FPS Camera ===
echo.

REM Step 1: Compile HEIDIC to C++
echo Compiling HEIDIC: examples\fps_camera\fps_camera.hd
cargo run -- compile examples\fps_camera\fps_camera.hd
if errorlevel 1 (
...
if not exist "examples\fps_camera\fps_camera.cpp" (
...
REM Change to fps_camera directory
cd /d "%SCRIPT_DIR%"
...
echo Compiling fps_camera.cpp...
g++ -std=c++17 -O3 %INCLUDES% -c fps_camera.cpp -o "%PROJECT_ROOT%\vulkan\obj\fps_camera.o"
if errorlevel 1 (
    echo Failed to compile fps_camera.cpp!
    exit /b 1
)
...
set "OBJ_FILES=%PROJECT_ROOT%\vulkan\obj\fps_camera.o %PROJECT_ROOT%\vulkan\obj\eden_vulkan_helpers.o"
...
set "LINK_CMD=!LINK_CMD! -o fps_camera.exe"
